#!/bin/bash
# Start script for Agent 3 (Documentation)

echo "🟣 Preparing Agent 3 (Documentation) prompt..."

# Generate combined prompt
PROMPT_FILE="/tmp/agent3-claude-prompt.txt"
cat /workspaces/terroir-core/.claude/agent-prompts/base/base-prompt.md \
    /workspaces/terroir-core/.claude/agent-prompts/agents/documentation-agent.md \
    > "$PROMPT_FILE"

# Add recovery context
echo -e "\n\n---\n\nI'm starting a fresh session. Please check the current state of documentation work and any pending tasks." >> "$PROMPT_FILE"
...
echo "   3. Agent 3 will initialize and check current state"
echo ""
echo "🟣 Agent 3 (Documentation) - Branch: feat/documentation"
