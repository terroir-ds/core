#!/bin/bash
# Start script for Agent 2 (Infrastructure)

echo "🔵 Preparing Agent 2 (Infrastructure) prompt..."

# Generate combined prompt
PROMPT_FILE="/tmp/agent2-claude-prompt.txt"
cat /workspaces/terroir-core/.claude/agent-prompts/base/base-prompt.md \
    /workspaces/terroir-core/.claude/agent-prompts/agents/infrastructure-agent.md \
    > "$PROMPT_FILE"

# Add recovery context
echo -e "\n\n---\n\nI'm starting a fresh session. Please check the current state of infrastructure development and any pending tasks." >> "$PROMPT_FILE"
...
echo "   3. Agent 2 will initialize and check current state"
echo ""
echo "🔵 Agent 2 (Infrastructure) - Branch: feat/infrastructure"
