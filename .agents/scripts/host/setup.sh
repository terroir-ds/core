...
if [ -d "$PARENT_DIR/terroir-shared" ]; then
    ...
    echo ""
    echo "   $REPO_DIR/.agents/scripts/migrate-from-shared.sh"
    echo ""
...
for i in 1 2 3; do
    ...
    if [ -d "../terroir-core/.devcontainer" ]; then
        ...
        if [ -f "../terroir-core/.agents/templates/devcontainer.json" ]; then
            cp "../terroir-core/.agents/templates/devcontainer.json" .devcontainer/devcontainer.json
